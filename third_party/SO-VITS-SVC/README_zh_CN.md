...
该脚本会为每个数据根目录（单说话人或多说话人）生成 `split.json`，并将训练配置切换为直接读取 split。
如需兼容旧流程，也可以添加 `--write_filelists` 额外导出 `filelists/train.txt` 与 `filelists/val.txt`。
