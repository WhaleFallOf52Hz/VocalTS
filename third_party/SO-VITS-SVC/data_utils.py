...
from dataset_manifest import (
    ...
    collect_samples,
    ensure_parent,
    load_legacy_filelist,
    resolve_feature_read_path,
)
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths, hparams, split: str = "train", all_in_mem: bool = False, vol_aug: bool = True):
        ...
        source_dir = getattr(hparams.data, "source_dir", None)
        split_file = getattr(hparams.data, "split_file", None)
        ...
        if source_dir:
            ...
            self.samples = load_legacy_filelist(audiopaths)
        ...
        if self.all_in_mem:
            self.cache = [self.get_audio(sample) for sample in self.samples]

    def get_audio(self, sample: SampleItem):
        filename = sample.audio_path.replace("\\", "/")
        ...
        spec_filename = resolve_feature_read_path(sample, "spec")

        # Ideally, all data generated after Mar 25 should have .spec.pt
        if os.path.exists(spec_filename):
            spec = _safe_torch_load(spec_filename)
        else:
            spec = spectrogram_torch(audio_norm, self.filter_length,
                                     self.sampling_rate, self.hop_length, self.win_length,
                                     center=False)
            spec = torch.squeeze(spec, 0)
            ensure_parent(spec_filename)
            torch.save(spec, spec_filename)

        spk = sample.speaker
        spk = torch.LongTensor([self.spk_map[spk]])

        f0_path = resolve_feature_read_path(sample, "f0")
        ...
        units_path = resolve_feature_read_path(sample, "units")
        c = _safe_torch_load(units_path)
        c = utils.repeat_expand_2d(c.squeeze(0), f0.shape[0], mode=self.unit_interpolate_mode)
        if self.vol_emb:
            volume_path = resolve_feature_read_path(sample, "volume")
    ...
    def __getitem__(self, index):
        if self.all_in_mem:
            ...
            return self.random_slice(*self.get_audio(self.samples[index]))
